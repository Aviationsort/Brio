  ...
  return (
    <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-2xl max-w-md mx-auto space-y-6 text-center">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        ...
        <button
          onClick={initGrid}
          className="skeuo-btn p-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl transition-all"
        >
          ...
          {grid.map((row) =>
            row.map((cell) => (
              <button
                ...
                className={`skeuo-btn w-7 h-7 rounded-lg text-xs font-bold transition-all flex items-center justify-center ${
                  cell.revealed
                    ? cell.isMine
