    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-2xl max-w-lg mx-auto space-y-6">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        ...
        <button
          ...
          className="skeuo-btn px-3 py-1 bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-red-300 rounded-xl transition-all"
        >
          Quick Pick
        </button>
      </div>
          ...
          {Array.from({ length: 30 }, (_, i) => i + 1).map((num) => {
            ...
            return (
              <button
                ...
                className={`skeuo-btn py-2.5 rounded-xl font-bold font-mono text-xs transition-all ${
                  isSelected
        ...
        <div className="bg-slate-950 p-4 rounded-2xl border border-slate-800 text-center space-y-2">
          ...
          <div className="flex justify-center gap-2">
            {drawnNumbers.map((n) => (
              <span
                key={n}
                className={`skeuo-btn w-9 h-9 rounded-full font-bold font-mono text-sm flex items-center justify-center ${
      ...
      <button
        ...
        className="skeuo-btn w-full py-3.5 bg-gradient-to-r from-rose-600 via-red-600 to-red-600 hover:from-rose-500 hover:to-red-500 text-white font-bold text-xs rounded-2xl shadow-xl transition-all flex items-center justify-center gap-2 disabled:opacity-40"
      >
