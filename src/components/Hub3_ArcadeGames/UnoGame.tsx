    ...
    <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-2xl max-w-xl mx-auto space-y-6">
      <div className="flex items-center justify-between border-b border-slate-800 pb-3">
        ...
        {!gameStarted ? (
          <button
            onClick={startUno}
            className="skeuo-btn px-3.5 py-1.5 bg-gradient-to-r from-rose-600 to-red-600 text-white text-xs font-bold rounded-xl shadow transition-all flex items-center gap-1.5"
          >
          ...
          <button
            onClick={startUno}
            className="skeuo-btn px-3 py-1 bg-slate-800 hover:bg-slate-700 text-xs text-slate-300 rounded-xl"
          >
            Reset Match
          </button>
        )}
        ...
        <div className="flex justify-center items-center gap-6 py-4">
          <div>
            ...
            <button
              ...
              className="skeuo-btn w-16 h-24 rounded-2xl bg-slate-800 border-2 border-slate-700 flex items-center justify-center font-bold text-white shadow-lg active:scale-95 transition-all"
            >
              DRAW
            </button>
          </div>
          ...
          <div>
            ...
            <div
              className={`skeuo-btn w-16 h-24 rounded-2xl flex flex-col items-center justify-center font-black text-sm shadow-2xl border-2 border-white/20 ${getBgColor(
                topCard.color
              )}`}
            >
        ...
        <div className="flex flex-wrap gap-2 justify-center max-h-48 overflow-y-auto p-2 bg-slate-950 rounded-2xl border border-slate-800">
          {playerHand.map((card) => (
            <button
              ...
              className={`skeuo-btn w-14 h-20 rounded-xl flex flex-col items-center justify-center font-black text-xs shadow-lg transition-transform hover:scale-110 border border-white/20 ${getBgColor(
                card.color
              )}`}
            >
